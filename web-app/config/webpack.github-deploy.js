...
const METADATA = webpackMerge(webpackConfig.metadata, {
   ...
   */
  baseUrl: '/' + GH_REPO_NAME + '/' + ghDeploy.safeUrl(webpackConfig.metadata
    .baseUrl)
});
...
module.exports = webpackMerge(webpackConfig, {
  ...
  output: {
     ...
     */
    publicPath: '/' + GH_REPO_NAME + '/' + ghDeploy.safeUrl(webpackConfig.output
      .publicPath)
  },
  ...
  plugins: [
    function() {
      this.plugin('done', function(stats) {
        ...
        ghpages.publish(webpackConfig.output.path, options, function(
          err) {
          if (err) {
